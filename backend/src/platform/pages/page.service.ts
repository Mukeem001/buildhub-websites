...
export const getPage = async (pageId: string) => {
  return WebsitePage.findById(new Types.ObjectId(pageId));
};
...
export const addSection = async (
  ...
  const page = await WebsitePage.findById(new Types.ObjectId(pageId));
...
export const updateSection = async (pageId: string, sectionId: string, data: any) => {
  const page = await WebsitePage.findById(new Types.ObjectId(pageId));
...
export const deleteSection = async (pageId: string, sectionId: string) => {
  ...
  page.sections = page.sections.filter((s: any) => s.id !== sectionId) as any;
  await page.save();

  return page;
};

export const reorderSections = async (pageId: string, sectionOrder: string[]) => {
  const page = await WebsitePage.findById(new Types.ObjectId(pageId));
